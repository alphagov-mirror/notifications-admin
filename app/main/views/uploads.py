    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if ex.response is not None and ex.response.status_code == 400:
                status = 'invalid'
                upload_letter_to_s3(pdf_file_bytes, file_location, status, page_count, original_filename)
            else:
                raise ex
        else:
            status = 'valid'
            file_contents = base64.b64decode(response.json()['file'].encode())
            upload_letter_to_s3(file_contents, file_location, status, page_count, original_filename)
